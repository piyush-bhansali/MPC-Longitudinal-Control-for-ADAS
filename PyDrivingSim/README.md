...
> Based on the original work by [Gastone Pietro Rosati Papini](https://github.com/tonegas/PyDrivingSim).
> Extended with a C++ agent library replacing the original auto-generated C code.

## Building the C++ agent library

The agent control library is written in C++ and must be compiled before running the simulator.
...
make
```
This produces `agent/libagent_lib.so`.

### macOS
```bash
cd agent/src
make TARGET=../libagent_lib.dylib
```

### Windows (MSVC)
```bash
cd agent/src
cl /LD agent_lib.cpp agent_compute.cpp /Fe:..\agent_lib.dll
```

To write your own ADAS algorithm, edit `agent/src/agent_compute.cpp` and rebuild with `make`.
